...
$page=new Page("centres/lookup.php","Centres");
...
<?PHP echo $page->drawMilestone(); ?>
		...
		if(!isset($_GET['centre'])) throw new Exception("No centre selected");
		
		$centre=new Centre((int)$_GET['centre']);
		...
		foreach($centre->sd as $local=>$field){
			if(in_array('id',$field['flags'])||in_array('fk',$field['flags'])||in_array('hidden',$field['flags'])){
				...
				$form[$local]=isset($centre->{$local})?$centre->{$local}:'';
			}
		}
		$errors=array();
		if(isset($_POST['formsubmit'])&&$_POST['formsubmit']==1){
				foreach($form as $f=>$v){
			try{
					$centre->set($f,(isset($_POST[$f]) ? $_POST[$f]:$form[$f]));}catch(Exception $e){
				$errors[]=$e->getMessage();
			}
		...
		?>
		  ...
		  ?>
      	<form action="?centre=<?PHP echo isset($_GET['centre'])?$_GET['centre']:''; ?>" method="post" id="save" class="form-horizontal">
        <fieldset>
          <legend>Edit Centre</legend>

          <?PHP echo $page->print_input("text","name","Name",$form["name"]); ?>
          <?PHP echo $page->print_input("tel","contacttel","Telephone",$form["contacttel"]); ?>
          <?PHP echo $page->print_input("bigtext","description","Description",$form["description"]); ?>
          <?PHP echo $page->print_input("bigtext","address","Address",$form["address"]); ?>
          ...
          <?PHP echo $page->print_input("select","status","Status",$form["status"],NULL,array("Approved"=>1,"Not approved"=>0)); ?>
